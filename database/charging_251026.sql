...
VALUES
('admin', MD5('admin'), 'admin', '18559113752', 1),/*管理员启用*/
('operator', MD5('operator'), 'operator', '13366167958', 1),/*操作人员启用*/
('viewer', MD5('viewer'), 'viewer', '13615978848', 1);/*只读用户*/

-- 更新现有用户的密码，之前9.1写错了没有写MD5+salt，现在改好了
# START TRANSACTION;
# UPDATE `charging_user` SET `password` = MD5(CONCAT('charge_salt_2025_', 'admin')) WHERE username = 'admin';
# UPDATE `charging_user` SET `password` = MD5(CONCAT('charge_salt_2025_', 'operator')) WHERE username = 'operator';
# UPDATE `charging_user` SET `password` = MD5(CONCAT('charge_salt_2025_', 'viewer')) WHERE username = 'viewer';
# COMMIT ;
